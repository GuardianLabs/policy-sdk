...
export class VariablesInserter implements IAsyncMapGetter<AllowedVariablesType> {
    ...
    private variablesValues: Map<string, {
        index: number,
        ...
        for(const expectedVariablesByNode of expectedVariablesConfig) {
            if(expectedVariablesByNode.variables) {
                for(const variable of expectedVariablesByNode.variables) {
                    this.nameToNodeId.set(variable.uniqueName, expectedVariablesByNode.nodeId);
    ...
    public get(variableUniqueName: string): AllowedVariablesType | undefined {
        ...
        return this.variablesValues.get(nodeId)!.find(el => el.index == this.nameToVariableIndex.get(variableUniqueName))?.value;
    }

    public getFilledVariables(): FilledVariables[] {
        let filledVariablesValues: FilledVariables[] = [];
        ...
        for(let [targetNodeId, filledVariablesByNode] of this.variablesValues) {
            ...
            const filledVariableValues = new Array<AllowedVariablesType>(filledVariablesByNode.length);
            for(let variableValueConfig of filledVariablesByNode) {
                filledVariableValues[variableValueConfig.index] = variableValueConfig.value;
            }
        ...
        filledVariablesValues = filledVariablesValues.concat(this.spareNodes.map(({nodeId}) => ({
            nodeId,
            values: []
        })));

        return filledVariablesValues;
    }
}